...
async function testLoginInvalidCredentials() {
  ...
  try {
    ...
    const errorAlert = await driver.findElement(
      By.css("div.alert.alert-danger"),
    );
    ...
    if (errorAlert) {
      ...
      throw new Error(
        `Ошибка при неверных пользовательских данных не была найдена`,
      );
    }
  } finally {
